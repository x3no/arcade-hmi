#!/usr/bin/env python3
"""
Arcade Control Panel - Desktop Test Version - CYBERPUNK THEME
Version for testing on desktop/laptop with X11
"""
...
# Don't set framebuffer for desktop testing
...
class MockUSBHID:
    """Mock USB HID for desktop testing"""
    
    def __init__(self, device_path='/dev/hidg0'):
        self.device_path = device_path
        print(f"[MOCK HID] Initialized {device_path}")
    ...
    def send_key(self, key_code, modifiers=0):
        print(f"[MOCK HID] Send key: {hex(key_code)} with modifiers: {modifiers}")


class MockGPIOController:
    """Mock GPIO for desktop testing"""
    
    def __init__(self, power_pin=17):
        self.power_pin = power_pin
        print(f"[MOCK GPIO] Initialized pin {power_pin}")
        
    def pulse_power_button(self, duration=0.2):
        print(f"[MOCK GPIO] Power button pulse for {duration}s")
        
    def force_shutdown(self, duration=5):
        print(f"[MOCK GPIO] Force shutdown - holding for {duration}s")
        ...
        if event.type == MOUSEMOTION:
            ...
            if self.rect.collidepoint(event.pos):
                if self.action:
                    self.action()
                return True
        return Falsetext_surf.get_rect(center=self.rect.center)
        surface.blit(text_surf, text_rect)
...
class ArcadeControlApp:
    ...
    def __init__(self):
        ...
        # Use windowed mode for desktop testing
        self.screen = pygame.display.set_mode((self.width, self.height))
        pygame.display.set_caption("CYBER//ARCADE - Control Panel")
        ...
        self.hid = MockUSBHID()
        self.gpio = MockGPIOController(self.config['gpio_power_pin'])
        ...
        print(f"PIN por defecto: {self.config['pin']}")
        print("1. Haz clic en la pantalla para despertar")
        print("2. Introduce PIN y pulsa OK (última fila)")
        print("="*50 + "\n")
    ...
    def numpad_press(self, key):
        ...
        if key == 'C':
            ...
            if self.pin_input == self.config['pin']:
                ...
                print(f"[APP] PIN incorrecto: {self.pin_input}")
                self.pin_input = ""
        else:
    ...
    def screen_off(self):
        ...
        print("[APP] Pantalla OFF")
        self.screen_on = False
        
    def wake_screen(self):
        """Wake screen from touch"""
        if not self.screen_on:
            print("[APP] Pantalla despertada")
            self.screen_on = True
    ...
    def volume_up(self):
        ...
        self.hid.send_key(ArcadeKeyMapper.get_key('volume_up'))
            
    def volume_down(self):
        """Decrease volume"""
        print("[ACTION] Volume DOWN")
        self.hid.send_key(ArcadeKeyMapper.get_key('volume_down'))
            
    def mute(self):
        """Toggle mute"""
        print("[ACTION] Mute/Unmute")
        self.hid.send_key(ArcadeKeyMapper.get_key('mute'))
            
    def coin_p1(self):
        """Insert coin for Player 1"""
        print("[ACTION] Coin Player 1")
        self.hid.send_key(ArcadeKeyMapper.get_key('coin_p1'))
            
    def coin_p2(self):
        """Insert coin for Player 2"""
        print("[ACTION] Coin Player 2")
        self.hid.send_key(ArcadeKeyMapper.get_key('coin_p2'))
            
    # Power actions with confirmation
    def power_on_confirm(self):
        """Show confirmation for power on"""
        print("[ACTION] Solicitando confirmación: Encender PC")
    ...
    def power_off_confirm(self):
        ...
        print("[ACTION] Solicitando confirmación: Apagar PC")
        self.confirmation_dialog = {
            'title': '¿Apagar PC?',
